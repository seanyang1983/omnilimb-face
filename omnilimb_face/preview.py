...
def _default_model_dict_path() -> Path:
    """Return the Live2D ``model_dict.json`` path.

    Shipped as package data inside ``omnilimb_face/models/`` so a pip-installed
    wheel (and the ``omnilimb-face`` console command) work without a source
    checkout. Falls back to the repo-root ``models/`` layout for source runs.
    """
    in_package = Path(__file__).resolve().parent / "models" / "model_dict.json"
    if in_package.is_file():
        return in_package
    return Path(__file__).resolve().parent.parent / "models" / "model_dict.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    # The front-end assets AND the Live2D model dictionary now ship as package
    # data inside omnilimb_face/, so a plain `pip install omnilimb-face` (and the
    # `omnilimb-face` console command) work without a source checkout. Both
    # resolvers fall back to the old repo-root layout for source runs.
    frontend_dir = default_frontend_dir()
    model_dict_path = _default_model_dict_path()
    ...
    async def _get_worker():
        ...
        async with worker_lock["lock"]:
            ...
            w = HermesWorker(
                args.hermes_python,
                args.hermes_dir,
                str(Path(__file__).resolve().parent.parent / "hermes_chat_worker.py"),
            )
