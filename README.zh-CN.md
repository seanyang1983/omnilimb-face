...
```bash
pip install "omnilimb-face[voice]"      # 麦克风采集 + VAD(免提)
pip install "omnilimb-face[wakeword]"   # 可选唤醒词
pip install "omnilimb-face[live2d]"     # 前端静态资源服务
pip install "omnilimb-face[preview]"    # 预览用的 Edge-TTS 语音 + 本地 STT
...
- **没有声音。** 装语音引擎:`pip install edge-tts`(或 `pip install "omnilimb-face[all]"`)。
