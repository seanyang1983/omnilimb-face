...
<p align="center">
  <a href="https://github.com/seanyang1983/omnilimb-face/blob/main/README.md">English</a> · <b>中文</b>
...
![omnilimb-face 演示](https://cdn.jsdelivr.net/gh/seanyang1983/omnilimb-face@main/docs/assets/demo.gif)

> 上图为合成预览(`omnilimb-face`)效果;静态截图见
> [`docs/assets/screenshot.png`](https://cdn.jsdelivr.net/gh/seanyang1983/omnilimb-face@main/docs/assets/screenshot.png)。
...
![架构图](https://cdn.jsdelivr.net/gh/seanyang1983/omnilimb-face@main/docs/assets/architecture.svg)
...
> 网页在 `网关端口 + 1` = **12394**。(单端口模式 `omnilimb-face --single-port` /
> `--https` 下,网页和网关才共用 12393。)

### 方式 B —— 完整形态(在 hermes 里聊天,形象说出真实回复)

```bash
# 1) 装进 hermes 的 venv(一条命令,全都包含)
...
```bash
omnilimb-face --lan --https              # 在局域网 IP 上以 HTTPS 提供服务(单端口 12393)
...
- **没有声音。** Edge-TTS 语音已随 `pip install omnilimb-face` 一起装好,所以多半是
  联网/手势问题:Edge 在线语音需要联网,且浏览器要求你先在页面里点一下/打个字才允许
  播放。预览启动时显示 `voice: on (edge-tts …)` 才表示语音引擎已启用。
- **装完了但没反应 / 看不到形象。** 安装只是装上代码,还得**启动**:`omnilimb-face`
...
见 [`LICENSE`](https://github.com/seanyang1983/omnilimb-face/blob/main/LICENSE)。
...
[`COMMERCIAL-LICENSE.md`](https://github.com/seanyang1983/omnilimb-face/blob/main/COMMERCIAL-LICENSE.md) 或联系 **yase19636404@163.com**。
...
[`NOTICE.md`](https://github.com/seanyang1983/omnilimb-face/blob/main/NOTICE.md) 与 [`THIRD_PARTY_NOTICES.md`](https://github.com/seanyang1983/omnilimb-face/blob/main/THIRD_PARTY_NOTICES.md)。
